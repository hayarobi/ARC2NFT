...
function new_arc2_nft(name, symbol, initial_supply, options, owner, ...)
    ...
    return contract.delegatecall(_implementation:get(), "new_arc2_nft", name, symbol, decimals, initial_supply, options, owner, ...)
end
function query(queryName, ...)
    assert(nil ~= queryName, "queryName is nil")
    assert(nil ~= _implementation:get(), "implementation is nil")

    return contract.delegatecall(_implementation:get(), queryName, ...)
end
...
abi.payable(default,invoke,new_arc2_nft)
